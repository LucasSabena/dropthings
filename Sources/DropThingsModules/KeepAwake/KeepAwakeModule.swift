...
public final class KeepAwakeModule: DropThingsModule {
    ...
    public func start() async throws {
        ...
        settings.durationMinutes = nil
        settings.activeUntil = nil
        persistSettings()
        applyState(true)
    ...
    public var menuBarPresentation: ModuleMenuBarPresentation? {
        ModuleMenuBarPresentation(
            ...
            togglesModuleLifecycle: true
        )
    }

    public var menuBarIconName: String {
        isAssertionActive ? "sun.max.fill" : "moon.zzz"
    }

    // Lifecycle toggles are owned by ModuleRegistry. Exposing a module-private
    // command here would recreate an enabled module with an inactive assertion.
